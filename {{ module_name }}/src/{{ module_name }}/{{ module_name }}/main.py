...
async def web_backend_runner() -> None:
    ...
    logger.info("web_backend_config", module_name=module_name, app_path=app_path)
    ...
    if os.path.exists(webserver_config_path):
        try:
            ...
            logger.warning(
                "webserver_config_load_failed", config_path=webserver_config_path, error=str(e)
            )
    else:
        logger.warning(
            "webserver_config_not_found", config_path=webserver_config_path, fallback="HTTP mode"
        )
    ...
    if ssl_enabled:
        logger.info("uvicorn_ssl_enabled", cert_path=cert_path, key_path=key_path)
        ...
        config = uvicorn.Config(
            app=app_path, host=host, port=port, log_level="info", log_config=None, reload=False
        )

    logger.info("uvicorn_starting", host=config.host, port=config.port, ssl_enabled=ssl_enabled)
...
async def initialize_module(taskmanager: TaskManager) -> tuple[VyraEntity, StateManager]:
    ...
    if not VYRA_SLIM:
        ...
        if entity.node is None:
            logger.critical(
                "ros2_node_missing", entity_module=entity.module_entry.name, slim_mode=VYRA_SLIM
            )
            raise RuntimeError("No ROS 2 node created in non-SLIM mode")

        taskmanager.add_task(ros_spinner_runner, entity)
        logger.info("ros2_spinner_task_scheduled", node_name=entity.node.get_name())
    else:
...
async def runner() -> None:
    ...
    """
    logger.info("runner_started", slim_mode=VYRA_SLIM, pid=os.getpid())
    ...
    try:
        ...
        logger.info("system_exit_received", exit_code=e.code, slim_mode=VYRA_SLIM)
        if taskmanager:
            logger.debug("cancelling_all_tasks", reason="system_exit")
            await taskmanager.cancel_all()

    except KeyboardInterrupt:
        logger.warning("keyboard_interrupt_received", pid=os.getpid())
        if taskmanager:
        ...
        if not VYRA_SLIM:
            if entity and hasattr(entity, "node") and entity.node is not None:
                try:
                    logger.debug("destroying_ros2_node", node_name=entity.node.get_name())
...
def main() -> None:
    ...
    try:
        ...
        logger.error("main_runtime_error", error=str(e), error_type=type(e).__name__)

    except Exception as e:
        log_exception(logger, e, context={"function": "main"})
...
if __name__ == "__main__":
    logger.info("module_direct_execution", file=__file__, pid=os.getpid())
    main()
